  ...
  <footer class="VPFooter">
    <div class="container">
      ...
      <p v-if="footerConfig.publicSecurityRecordCode" class="recordCode">
        ...
        <span class="content">
          <a
            :href="`http://www.beian.gov.cn/portal/registerSystemInfo?recordcode=${footerConfig.publicSecurityRecordCode.replace('号', '').substring(footerConfig.publicSecurityRecordCode.indexOf('备') + 1)}`"
            target="_blank"
          >{{ footerConfig.publicSecurityRecordCode }}</a>
        </span>
      </p>
      <p v-if="footerConfig.copyright" class="copyright" v-html="footerConfig.copyright" />
    </div>
  </footer>
</template>
